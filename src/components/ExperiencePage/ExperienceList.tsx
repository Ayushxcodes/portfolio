      ...
      {experience.map((job, i) => (
        ...
        >
          <div className="md:sticky md:top-28 h-fit">
            <div className="w-12 h-12 bg-[#141414] border border-[#222] flex items-center justify-center mb-4 font-['JetBrains_Mono',monospace] text-[0.65rem] text-[#d4ff47] tracking-[0.05em]">{job.logo}</div>
          ...
          <div>
            <div className="font-['DM_Serif_Display',serif] text-[2rem] mb-[0.5rem] leading-[1.1]">{job.role}</div>
            <p className="font-['JetBrains_Mono',monospace] text-[0.78rem] text-[#aaa] leading-[2] mb-6 max-w-[540px]">{job.desc}</p>
            ...
            <div className="flex flex-wrap gap-[0.4rem]">
              {job.tags.map((t: string) => (
                <span key={t} className="font-['JetBrains_Mono',monospace] text-[0.65rem] border border-[#222] text-[#6a6a6a] px-[0.65rem] py-[0.25rem] tracking-[0.08em]">{t}</span>
              ))}
            </div>
          </div>
        </div>
