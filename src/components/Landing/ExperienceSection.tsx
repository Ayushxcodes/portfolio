  ...
  return (
    <section className="px-4 md:px-12 py-20 md:py-28 border-b border-[#222]" id="experience">
      ...
      <div className="flex flex-col">
        {experience.map((exp) => (
          <div key={exp.company} className="fade-up grid gap-6 md:gap-12 py-8 md:py-10 border-b border-[#222] grid-cols-1 md:grid-cols-[200px_1fr]">
            <div>
            ...
            <div>
              <div className="font-serif text-[1.25rem] md:text-[1.6rem] mb-3">{exp.role}</div>
              <div className="font-mono text-[0.75rem] text-[#6a6a6a] leading-[1.8] max-w-full md:max-w-[520px]">{exp.desc}</div>
              {exp.achievements && (
