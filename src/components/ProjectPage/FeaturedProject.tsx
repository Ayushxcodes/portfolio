...
export default function FeaturedProject({ stats, tags, details, onHover, onLeave }: { stats: StatItem[]; tags: string[]; details: FeaturedProjectDetails; onHover: () => void; onLeave: () => void; }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 border-b border-[#222] min-h-[500px]">
      ...
      <div className="px-14 py-14 flex flex-col justify-center">
        <div className="font-['JetBrains_Mono',monospace] text-[0.65rem] text-[#d4ff47] tracking-[0.18em] uppercase mb-5">{details.cat}</div>
        <h2 className="font-['DM_Serif_Display',serif] text-[clamp(2rem,4vw,3.5rem)] leading-[1.05] mb-4">{details.title}</h2>
        <p className="font-['JetBrains_Mono',monospace] text-[0.8rem] text-[#6a6a6a] leading-[1.95] mb-8">{details.desc}</p>
